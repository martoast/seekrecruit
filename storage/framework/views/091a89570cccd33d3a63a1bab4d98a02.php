<?php
    $flashes = [];
    foreach (['success', 'error', 'warning', 'info'] as $type) {
        if (session()->has($type)) {
            $flashes[] = ['type' => $type, 'message' => session($type)];
        }
    }
    if (session()->has('status')) {
        $flashes[] = ['type' => 'info', 'message' => session('status')];
    }
?>
<?php if(count($flashes)): ?>
    <div class="fixed top-4 right-4 z-50 space-y-3 max-w-md" data-toast-container>
        <?php $__currentLoopData = $flashes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $flash): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
                $classes = match ($flash['type']) {
                    'success' => 'bg-emerald-50/95 text-emerald-800 border-emerald-200',
                    'error' => 'bg-red-50/95 text-red-800 border-red-200',
                    'warning' => 'bg-amber-50/95 text-amber-800 border-amber-200',
                    default => 'bg-blue-50/95 text-blue-800 border-blue-200',
                };
            ?>
            <div class="flex items-start p-4 rounded-xl shadow-lg border backdrop-blur-sm <?php echo e($classes); ?>" data-toast>
                <div class="flex-shrink-0 mt-0.5">
                    <?php if($flash['type'] === 'success'): ?>
                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                    <?php elseif($flash['type'] === 'error'): ?>
                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/></svg>
                    <?php elseif($flash['type'] === 'warning'): ?>
                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                    <?php else: ?>
                        <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/></svg>
                    <?php endif; ?>
                </div>
                <div class="ml-3 flex-1">
                    <p class="text-sm font-medium"><?php echo e($flash['message']); ?></p>
                </div>
                <button type="button" class="ml-4 flex-shrink-0 text-gray-400 hover:text-gray-600 transition-colors" data-toast-close>
                    <svg class="h-5 w-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/></svg>
                </button>
            </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
<?php endif; ?>
<?php /**PATH /Volumes/Seagate/Max/Seekrecruit/seekrecruit/resources/views/partials/toast.blade.php ENDPATH**/ ?>